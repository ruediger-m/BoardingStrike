...
- **Step 6 — Godot board view:** complete. `BoardView` renders the map terrain (flat-top hexes, walls, doors) using `Core.HexLayout` as the shared geometry source, with an F1 coordinate overlay.
- **Step 7 — live units:** complete. The scenario auto-plays round by round via the Game-layer `AutoMarineController`, and `ScenarioScene` animates the rules-engine **event stream**: `UnitView` nodes tween along moves, flash on hits, update HP bars, show condition dots, fade out on death; doors redraw on change; a HUD log shows the play-by-play; Space pauses, R restarts. Combat events now carry resulting HP so the bars stay accurate. Verified to compile against the Godot 4.6 API; 75 engine tests green; visual confirmation is done in-editor.

Next up is **Step 8** (player input: a card-hand HUD and click-to-play turns, replacing the auto controller).
