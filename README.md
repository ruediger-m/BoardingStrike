...
- **Step 4 — Hangar Sweep content:** complete. Authored the full MVP content set under `godot/data/`: the 10 Boarding Marine cards, the `boarding_marine` class, the `husk_swarmer`/`cyst_spitter` enemies and their 16 AI cards, the two modifier decks, and the `map_derelict_alpha` map + `mission_hangar_sweep` mission. A test harness builds an `IHexGraph` from the map and verifies every hostile is reachable from every marine spawn with doors open, and that each gated room is sealed when its door is closed. 62 unit tests green.

Next up is **Step 5** (board state + rules engine: turn/round loop, combat resolution, conditions, refresh/exhaustion, enemy AI execution).
